    ...
    public class UserProfileService : IUserProfileService
    {
        ...
        public virtual EditProfile GetProfile(UserProfile userProfile)
        {
            ...
            var model = new EditProfile
            {
                ...
                InterestTypes = this.profileSettingsService.GetInterests(),
                ProductName = GetProduct(properties)
            };
        ...
        private string GetProduct(IDictionary<string,string> dictionary)
        {
            string productName = string.Empty;
            string ProductPageID = dictionary.ContainsKey("ProductId") ? dictionary["ProductId"] : "";
            if (!string.IsNullOrEmpty(ProductPageID))
            {
                Item itm = Sitecore.Context.Item.Database.GetItem(new Sitecore.Data.ID(ProductPageID));
                if (itm != null)
                {
                    productName = itm.Fields["Title"] != null ? itm.Fields["Title"].Value : itm.Name;
                }
            }
